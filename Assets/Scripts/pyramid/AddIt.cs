...
public class AddIt : GGame
{
    ...
    void Start()
    {
        ...
        if (level < 3)
        {
            ...
            if (timer_to_finish <= 30)
            {
                njom = 3;
                for (int i = 0; i < njom_sofar.Length; i++) njom_sofar[i].SetActive(true);
            }
            else if (timer_to_finish <= 60 && timer_to_finish >= 30)
            {
                njom = 2;
            ...
            if(PlayerPrefs.GetInt("id_user")!=0)
                add_Score_Db.UpdateData( PlayerPrefs.GetInt("id_user")   ,  11  ,  njom  , timer_to_finish);
        }
